...
from gvpgnn.gvp import GVP, GVPConvLayer, LayerNorm
  ...
  * Paper: https://arxiv.org/abs/2009.01411
  ...
  def __init__(
    ...
    self.dense = nn.Sequential(
      nn.Linear(self.pooling_op.get_output_dim() + 3*ns, 2*ns*width_multiplier),
      nn.ReLU(inplace=True),
      nn.Dropout(p=drop_rate),
      nn.Linear(2*ns*width_multiplier, 2*ns),
      nn.ReLU(inplace=True),
      nn.Dropout(p=drop_rate),
