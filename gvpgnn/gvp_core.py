...
class GVP(nn.Module):
  ...
  See manuscript and README.md for more details.
...
class GVPConv(MessagePassing):
  ...
  * `aggr`: should be "add" if some incoming edges are masked, as in a masked
    autoregressive decoder architecture, otherwise "mean"
...
class GVPConvLayer(nn.Module):
  ...
  a pointwise feedforward network to node embeddings, and returns updated node
  embeddings.
  ...
  * `autoregressive`: if `True`, this `GVPConvLayer` will be used with a different
      set of input node embeddings for messages where src >= dst
  * `activations`: tuple of functions (scalar_act, vector_act) to use in GVPs
