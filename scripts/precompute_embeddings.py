...
if __name__ == "__main__":
  ...
  dataset_folder = paths.data_folder("cleaned_skip_missing")
  ...
  for split_name in ("train", "test", "val"):
    ...
    json_files = glob.glob(os.path.join(dataset_folder, split_name, "*.json"))

    for filename in json_files:
      with open(filename, "r") as f:
        data = dm.ProteinBackboneWithEmbedding.model_validate(json.load(f))
      
      # Use the "unk" token for missing/bad residues.
      seq = data.seq.replace("_", "<unk>")
      features = plm.extract_embedding_single(model, alphabet, feat_layer, seq, device=device)
